...
export default function FichaAvaliacaoPar() {
	...
	const enviarAvaliacao = async () => {
		...
		if (!avaliadoId) {
			alert("Selecione o profissional a ser avaliado.");
			return;
		}
	...
	useEffect(() => {
		carregar(
			`/api/criterios-avaliacao-par/${tipoAvaliacao}`,
			setCriterios
		);
	}, [tipoAvaliacao]);
									...
									<div className='flex items-center gap-4 p-5'>
										...
										<div className='flex-1'>
											<div className='flex items-center gap-2 flex-wrap'>
												...
												<span className='bg-[#cd0048]/20 text-white/80 text-xs px-2 py-0.5 rounded-full font-medium border border-[#cd0048]/30'>{tipoAvaliacao}</span>
											</div>
									...
									<div className='bg-[#061c31]/50 px-5 py-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 print:grid-cols-4'>
										...
										<div>
											...
											<input
												...
												value={usuarios.find(u => String(u.id) === String(avaliadoId))?.nome || "Nenhum avaliado selecionado"}
												disabled
											/>
										</div>
									</div>
